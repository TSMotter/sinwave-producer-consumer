...
class _RealWebSocket : public easywsclient::WebSocket
{
   public:
    ...
    void sendData(wsheader_type::opcode_type type, uint64_t message_size, Iterator message_begin,
                  Iterator message_end)
    {
        ...
        header.assign(
            2 + (message_size >= 126 ? 2 : 0) + (message_size >= 65536 ? 6 : 0) + (useMask ? 4 : 0),
            0);
        header[0] = 0x80 | type;
        if (false)
        {
    ...
    void close()
    {
        ...
        readyState                         = CLOSING;
        uint8_t              closeFrame[6] = {0x88, 0x80, 0x00,
                                              0x00, 0x00, 0x00};  // last 4 bytes are a masking key
